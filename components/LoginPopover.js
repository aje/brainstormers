...
const LoginPopover = ({providers}) => {
	...
	const onSocial = provider => () => {
		setLoading(provider);
		signIn(provider).then(r => setLoading(null));
	};
			...
			<Popover isOpen={state.get()} onOpenChange={state.set}>
				...
				<Popover.Content className={"p-5 w-72"}>
					...
					<Button
						...
						disabled={!!loading}
						icon={<Google size={22} fill="currentColor" />}
						bordered
						className={"w-full"}>
						{loading === "google" ? <Loading type="points-opacity" color="currentColor" size="sm" /> : "Google Login"}
					</Button>
					<Button
						onClick={onSocial("github")}
						disabled={!!loading}
						icon={<Github size={22} fill="currentColor" />}
						bordered
						className={"w-full mt-2"}>
						{loading === "github" ? <Loading type="points-opacity" color="currentColor" size="sm" /> : "Github Login"}
					</Button>
					{/*</div>}*/}
				</Popover.Content>
			</Popover>
