    ...
    return (<div className={"py-20 bg-gray-100"} style={{backgroundImage: "url(/ent.png)", backgroundSize: "cover", backgroundPosition:"bottom"}}>
        <Container >
            <main >
                ...
                <h3>Last month <span className="font-normal"> most active entrepreneurs({countTopEnt})</span></h3>
                <Grid.Container gap={2}>
                    {users.map((user, i) => <Grid key={user.id} xs={12} sm={4} className={"flex flex-col text-center items-center mt-4"} >
                        <User size={"2xl"}   src={user.image} name={''}/>
                        <h3 className={"mt-4"}>{user.name} {user.role && <span className={"capitalize border-gray-300 text-gray-400 text-xs font-normal border-solid border bg-amber-50 rounded-md px-2"}>{user.role}</span>}</h3>
